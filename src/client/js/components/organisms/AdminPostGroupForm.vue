  ...
  methods: {
    ...
    async save() {
      ...
      try {
        ...
        let res
        if (this.isEdit) {
          res = await Admin.updatePostGroup(this.serviceId, this.group.slug, vals, this.adminUserToken)
        } else {
    ...
    async checkSlugNotExists(slug) {
      try {
        this.$store.dispatch('setLoading', true)
        const res = await Admin.checkPostGroupSlugNotExists(this.serviceId, slug, this.adminUserToken)
        this.$store.dispatch('setLoading', false)
        return res
      } catch (err) {
        console.log(err);//!!!!!!
